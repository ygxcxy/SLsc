        ...
        <form class="layui-form" action="">
            <div class="layui-form-item">
                <div class="layui-input-block">
                    <input type="text" name="goodsPackCode" required  lay-verify="required" placeholder="请输入关键字" autocomplete="off" class="layui-input">
                </div>
            </div>
...
<script>
    layui.use(['table','jquery','table','layer'], function(){
        ...
        function main() {
            table.render({
                elem: '#test'
                ,url: '${ctx}/sys/pack/list' //数据接口
                ,page: true //开启分页
                ,limit:5
                ,toolbar: '#toolbarDemo' //开启头部工具栏，并为其绑定左侧模板
                ,cols: [[ //表头
                    {type: 'checkbox', fixed: 'left'}
                    ,{field: 'goodsPackCode', title: '套餐编码', width:180, sort: true, fixed: 'left'}
                    ,{field: 'goodsPackName', title: '套餐名称', width:180}
                    ,{field: 'realPrice', title: '零售价(元)', width:180
                        ,templet: function(d){
                ...
                where:{
                    goodsPackCode:$("input[name='goodsPackCode']").val()
                }
            });
        }
    });
